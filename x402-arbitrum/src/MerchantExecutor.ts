...
> = {
  ...
  'arbitrum-sepolia': {
    chainId: 421614,
    assetAddress: '0x75faf114eafb1BDbe2F0316DF893fd58Ce87AAf7',
    assetName: 'USDC',
...
export class MerchantExecutor {
  ...
  constructor(options: MerchantExecutorOptions) {
    ...
    const normalizedAssetAddress = ethers.getAddress(assetAddress.toLowerCase());
    const normalizedPayToAddress = ethers.getAddress(options.payToAddress.toLowerCase());

    this.requirements = {
      scheme: 'exact',
      network: options.network,
      asset: normalizedAssetAddress, // Properly checksummed
      payTo: normalizedPayToAddress, // Properly checksummed
